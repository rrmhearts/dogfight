...
class Aircraft:
    ...
    def execute_bomb_target(self, dt, maneuver, ground_targets):
        ...
        if maneuver.target and maneuver.target.alive:
            ...
            if self.maneuver_state['phase'] == 'approach':
                ...
                # Check realistic CCIP firing solution if bomb is selected
                weapon = self.weapons[self.current_weapon]
                if weapon.weapon_type == WeaponType.BOMB and self.ccip is not None:
                    dist_to_impact = np.linalg.norm(self.ccip - target.pos[:2])
                    if dist_to_impact < 30 and self.can_fire_at_target(target, is_ground=True):
                        proj = self.try_fire_weapon()
                        if proj:
                            self.maneuver_state['phase'] = 'egress'
                            return proj
                # Fallback for other ground-capable weapons (cannons)
    ...
    def adjust_altitude(self, desired_altitude, dt):
        ...
        if alt_diff > 20: self.pitch = min(max_pitch, self.pitch + dt * 0.5)
        elif alt_diff < -20: self.pitch = max(-max_pitch, self.pitch - dt * 0.5)
        else: self.pitch *= 0.95  # Level out
...
class DogfightSimulation:
    ...
    def draw_ground_target(self, target):
        ...
        if target.target_type == "aa_gun" and target.can_shoot and abs(target.turret_angle - target.target_turret_angle) > 0.1:
            ...
            for i in range(3):
                ...
                if abs(angle_step) > math.pi: angle_step = angle_step - 2*math.pi if angle_step > 0 else angle_step + 2*math.pi
    ...
    def draw_aircraft(self, aircraft):
        ...
        # Draw CCIP if selected and bomb is active
    ...
    def draw_projectile(self, projectile):
        if not projectile.active: return
        x, y, _ = self.project_3d_to_2d(projectile.pos)
        
        if projectile.config.weapon_type == WeaponType.MACHINE_GUN: color, size = (255, 255, 0), 2
        elif projectile.config.weapon_type == WeaponType.MISSILE: color, size = (255, 100, 0), 4
        elif projectile.config.weapon_type == WeaponType.BOMB: color, size = (255, 150, 100), 6
        else: color, size = (255, 200, 0), 3
